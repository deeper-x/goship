...
func GetAllRoadstead(idPortinformer string) []map[string]string {
	...
	query := `SELECT id_control_unit_data, ship_description, 
...
AND control_unit_data.fk_portinformer = $2`
	...
	rows, err := connector.Query(query, idPortinformer, idPortinformer)
...
func GetArrivalPrevisions(idPortinformer string) []map[string]string {
	...
	query := `SELECT ship_description AS ship, ts_arrival_prevision,
...
AND planned_arrivals.fk_portinformer = $1`
	...
	rows, err := connector.Query(query, idPortinformer)
...
func GetDeparturePrevisions(idPortinformer string) []map[string]string {
	...
	query := `SELECT ship_description AS ship, ts_departure_prevision,
ship_types.type_acronym AS ship_type,  
countries.iso3 AS ship_flag,
ships.width AS ship_width,
ships.length AS ship_length,
...
AND ts_avvistamento::DATE = current_date`
	...
	rows, err := connector.Query(query, idArrivalPrevision, idArrivalPrevision, idArrivalPrevision, idArrivalPrevision, idPortinformer)
...
AND ts_out_of_sight::DATE = current_date`
	...
	rows, err := connector.Query(query, idDepartureState, idPortinformer)
